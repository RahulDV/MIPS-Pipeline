...
int main(int argc, char* args[]) {
  ...
  while (continueSimulation && (clockCycle <= TOTALCYCLE)) {
	for(i=TOTAL_STAGE_TYPE-1;i>=0;i--){
		AbstractStage *stage = stages[i];
		Instruction cins = stage->getInstruction();
		if(i==0){
			if(!hault){
				stage->process();
				cins = stage->getInstruction();
			        if(cins.isHlt() || cins.isBranch())
					hault=true;
			}else{
				stage->setInstruction(*(new Instruction()));
			}
		}else if(!cins.isNop()&&!cins.isHlt()){
			if(i==3){
				if(cins.isBranch())
					hault=false;
			}
				...
				int j;
				for(j=3;j>i;j--){
					AbstractStage *destStage = stages[i];
					Instruction dest = destStage->getInstruction();
					AbstractStage *srcStage = stages[j];
			        	Instruction src = srcStage->getInstruction();
					if(!src.isNop() && !src.isHlt() && Instruction::isDataDependent(src,dest)){
						RegisterType srcWType = src.getArg1();
                				if(dest.isStore()){
			                        	if(dest.getArg2()==srcWType){
                        			        	dest.setSrcCycle1(src.getFetchedAtCycle());
                               				}
                        				if(dest.getArg1()==srcWType){
                				...
                				}else if(dest.isLoad() || dest.isAluImm()){
                         				if(dest.getArg2()==srcWType){
                                				dest.setSrcCycle1(src.getFetchedAtCycle());
                             	                        }
                				}else{
                        				...
                        				if(dest.getArg3()==srcWType){
                                				dest.setSrcCycle2(src.getFetchedAtCycle());
                                                        }
                				}
						destStage->setInstruction(dest);	
					}
				}
			}
			stage->process();
	...
	AbstractStage *wstage = stages[4];
	Instruction wins = wstage->getInstruction();
	if(wins.isHlt())
		continueSimulation=false;
	if(wins.isNop())
		incStatistics(STALLCYCLE);
	else
		incStatistics(FINISHEDINS);//finishedIns++
	for(i=TOTAL_STAGE_TYPE-1;i>0;i--){
		AbstractStage *stage = stages[i];
		AbstractStage *previousStage = stage->getPrevStage();
		Instruction cins = stage->getInstruction();
		Instruction pins = previousStage->getInstruction();
		if(!cins.isHlt()){
			if(!previousStage->isStalled()){
				stage->setInstruction(pins);
			}else if(!stage->isStalled()){
				stage->setInstruction(*(new Instruction()));
			}
		}
	}
  ...
  cout << "Name: Venkat Rahul Dantuluri, ID: fpj626" << endl;
